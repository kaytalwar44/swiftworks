-- =============================================================================
-- SwiftWorks - MASTER SCHEMA
-- Complete PostgreSQL / Supabase schema, assembled from 27 ordered migrations.
--
-- Target:   PostgreSQL 15+ (Supabase)
-- Run as:   a single transaction-safe script:  psql -f swiftworks_master_schema.sql
--
...
-- ---------------------------------------------------------------------------
-- 3. user_invitations — the only legitimate path to tenant membership
-- ---------------------------------------------------------------------------
...
-- ---------------------------------------------------------------------------
-- 4. Tenant bootstrap — creates company + owner role + owner membership
-- ---------------------------------------------------------------------------
...
create index if not exists booking_rate_limit_bucket_idx on public.booking_rate_limit (bucket, occurred_at desc);
...
--        from pg_class c join pg_namespace n on n.oid = c.relnamespace
--        where n.nspname = 'public' and c.relkind = 'r';
--   2. Confirm every tenant table has policies:
--        select tablename, count(*) from pg_policies
--        where schemaname = 'public' group by tablename order by 1;
--   3. Schedule public.booking_housekeeping() on cron every 5 minutes.
--   4. Seed subscription_plans from the Stripe product catalogue.
--   5. Regenerate types:
